/* eslint-disable */
// this is an auto generated file. This will be overwritten

export const createSDK = /* GraphQL */ `
  mutation CreateSDK(
    $input: CreateSDKInput!
    $condition: ModelSDKConditionInput
  ) {
    createSDK(input: $input, condition: $condition) {
      id
      SDKLk
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
export const updateSDK = /* GraphQL */ `
  mutation UpdateSDK(
    $input: UpdateSDKInput!
    $condition: ModelSDKConditionInput
  ) {
    updateSDK(input: $input, condition: $condition) {
      id
      SDKLk
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
export const deleteSDK = /* GraphQL */ `
  mutation DeleteSDK(
    $input: DeleteSDKInput!
    $condition: ModelSDKConditionInput
  ) {
    deleteSDK(input: $input, condition: $condition) {
      id
      SDKLk
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
export const createUserData = /* GraphQL */ `
  mutation CreateUserData(
    $input: CreateUserDataInput!
    $condition: ModelUserDataConditionInput
  ) {
    createUserData(input: $input, condition: $condition) {
      id
      email
      activationID
      timeStamp
      PULSE_RATE
      PULSE_RATE_CONF_LEVEL
      BLOOD_PRESSURE
      HEMOGLOBIN
      LF_HF
      OXYGEN_SATURATION
      STRESS_LEVEL
      WELLNESS_INDEX
      userIPAdd
      userDeviceInfo
      userDeviceInfoVersion
      SDNN
      SDNN_CONF_LEVEL
      MEAN_RRI
      MEAN_RRI_CONF_LEVEL
      RESPIRATION_RATE
      RESPIRATION_RATE_CONF_LEVEL
      PRQ
      PRQ_CONF_LEVEL
      RRI
      RRI_CONF_LEVEL
      HEMOGLOBIN_A1C
      PNS_INDEX
      PNS_ZONE
      RMSSD
      SD1
      SD2
      SNS_INDEX
      SNS_ZONE
      STRESS_INDEX
      WELLNESS_LEVEL
      ASCVD_RISK
      HEART_AGE
      HIGH_BLOOD_PRESSURE_RISK
      HIGH_FASTING_GLUCOSE_RISK
      HIGH_HEMOGLOBIN_A1C_RISK
      HIGH_TOTAL_CHOLESTEROL_RISK
      LOW_HEMOGLOBIN_RISK
      NORMALIZED_STRESS_INDEX
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
export const updateUserData = /* GraphQL */ `
  mutation UpdateUserData(
    $input: UpdateUserDataInput!
    $condition: ModelUserDataConditionInput
  ) {
    updateUserData(input: $input, condition: $condition) {
      id
      email
      activationID
      timeStamp
      PULSE_RATE
      PULSE_RATE_CONF_LEVEL
      BLOOD_PRESSURE
      HEMOGLOBIN
      LF_HF
      OXYGEN_SATURATION
      STRESS_LEVEL
      WELLNESS_INDEX
      userIPAdd
      userDeviceInfo
      userDeviceInfoVersion
      SDNN
      SDNN_CONF_LEVEL
      MEAN_RRI
      MEAN_RRI_CONF_LEVEL
      RESPIRATION_RATE
      RESPIRATION_RATE_CONF_LEVEL
      PRQ
      PRQ_CONF_LEVEL
      RRI
      RRI_CONF_LEVEL
      HEMOGLOBIN_A1C
      PNS_INDEX
      PNS_ZONE
      RMSSD
      SD1
      SD2
      SNS_INDEX
      SNS_ZONE
      STRESS_INDEX
      WELLNESS_LEVEL
      ASCVD_RISK
      HEART_AGE
      HIGH_BLOOD_PRESSURE_RISK
      HIGH_FASTING_GLUCOSE_RISK
      HIGH_HEMOGLOBIN_A1C_RISK
      HIGH_TOTAL_CHOLESTEROL_RISK
      LOW_HEMOGLOBIN_RISK
      NORMALIZED_STRESS_INDEX
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
export const deleteUserData = /* GraphQL */ `
  mutation DeleteUserData(
    $input: DeleteUserDataInput!
    $condition: ModelUserDataConditionInput
  ) {
    deleteUserData(input: $input, condition: $condition) {
      id
      email
      activationID
      timeStamp
      PULSE_RATE
      PULSE_RATE_CONF_LEVEL
      BLOOD_PRESSURE
      HEMOGLOBIN
      LF_HF
      OXYGEN_SATURATION
      STRESS_LEVEL
      WELLNESS_INDEX
      userIPAdd
      userDeviceInfo
      userDeviceInfoVersion
      SDNN
      SDNN_CONF_LEVEL
      MEAN_RRI
      MEAN_RRI_CONF_LEVEL
      RESPIRATION_RATE
      RESPIRATION_RATE_CONF_LEVEL
      PRQ
      PRQ_CONF_LEVEL
      RRI
      RRI_CONF_LEVEL
      HEMOGLOBIN_A1C
      PNS_INDEX
      PNS_ZONE
      RMSSD
      SD1
      SD2
      SNS_INDEX
      SNS_ZONE
      STRESS_INDEX
      WELLNESS_LEVEL
      ASCVD_RISK
      HEART_AGE
      HIGH_BLOOD_PRESSURE_RISK
      HIGH_FASTING_GLUCOSE_RISK
      HIGH_HEMOGLOBIN_A1C_RISK
      HIGH_TOTAL_CHOLESTEROL_RISK
      LOW_HEMOGLOBIN_RISK
      NORMALIZED_STRESS_INDEX
      createdAt
      updatedAt
      _version
      _deleted
      _lastChangedAt
      owner
      __typename
    }
  }
`;
